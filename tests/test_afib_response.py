...
def test_staging_probability_band_remains_authoritative_for_nested_head():
    doc = {'outcome': 'ACCEPT', 'predicted_class': 'SINUS',
           'afib_probability': 0.45, 'afib_result': 'INCONCLUSIVE',
           'afib_result_basis': {'category': 'rhythm', 'why': 'inside decision band'},
           'head_results': [{'head': 'afib', 'value': {'afib_result': 'AFIB_NOT_DETECTED'}}]}
    out = finalize_afib_response(doc)
    assert out['afib_result'] == 'INCONCLUSIVE'
    assert out['head_results'][0]['value']['afib_result'] == 'INCONCLUSIVE'
